...
if save_visualization:
    os.makedirs("_data/vtk_frames/shell_animation", exist_ok=True)
    folder_path = Path("_data/vtk_frames/shell_animation")
...
while True:
    ...
    if not k % nth_step_recorded:
        if save_visualization:
            writevtk(
                f"_data/vtk_frames/shell_animation/frame_{int(k/nth_step_recorded)}.vtk", system)
...
if save_visualization:
    ...
    plt.savefig("_data/shell_relaxation.png")
...
# Empty folder for recording frames for Paraview
if save_visualization:
    os.makedirs("_data/vtk_frames/inside_animation", exist_ok=True)
    folder_path = Path("_data/vtk_frames/inside_animation")
    for item in folder_path.iterdir():
        if item.is_file():
            item.unlink()
...
if save_visualization:
    ...
    plt.savefig("_data/max_forces.png")
...
filename = "_data/raspberry_coordinates" + str(AXES).replace(" ", "_") + ".txt"
